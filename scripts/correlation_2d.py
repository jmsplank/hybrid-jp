...
# %%
def corr_len_1d(arr: hj.arrfloat, dx: float) -> float:
    arr -= arr.mean()
...
def corr_len_2d(arr: hj.arrfloat, dx: float, dy: float) -> tuple[float, float]:
    cx = corr_len_1d(arr[:, 0], dx)
    cy = corr_len_1d(arr[0, :], dy)
    return cx, cy


print(corr_len_2d(test_b[:, :, 0], cs.dx, cs.dy))
